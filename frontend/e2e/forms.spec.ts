...
test('the block form opens empty after an edit was cancelled', async ({ page }) => {
	...
	await expect(page.getByLabel('Start')).toHaveValue('12:00');
	...
	await expect(page.getByLabel('End')).toHaveValue('10:00');
...
test('the add-time form opens empty after an edit was cancelled', async ({ page }) => {
	...
	await expect(page.getByLabel('Start')).toHaveValue('11:00');
	await expect(page.getByLabel('Note')).toHaveValue(/ingest step/);
	await page.getByRole('button', { name: 'Cancel' }).click();
